...
for a in range(len(multi_distance)):
    ...
    for t in range(T):
        for i in range(total_UE):
            trajectory_x[t, i] = np.random.normal(loc=0, scale=1) * multi_distance[a]
            trajectory_y[t, i] = np.random.normal(loc=0, scale=1) * multi_distance[a]
